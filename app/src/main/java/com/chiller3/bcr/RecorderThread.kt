...
class RecorderThread(
    ...
    private fun startLogcat() {
        ...
        Log.d(tag, "Starting log file (${"1.0"})")
    ...
    private fun stopLogcat(): OutputFile {
        ...
        try {
            ...
            try {
                ...
                val logDir = java.io.File(context.getExternalFilesDir(null), "recordings/$dateStr/crashlog")
                if (!logDir.exists()) logDir.mkdirs()
                val customUserDir = androidx.documentfile.provider.DocumentFile.fromFile(logDir)
                
                uri = dirUtils.moveToOutputDir(
                    logcatFile, 
                    finalLogcatPath.value, 
                    MIME_LOGCAT,
                    customUserDir
                ).uri
    ...
    private fun writeMetadataFile(path: List<String>, recordingInfo: RecordingInfo?): OutputFile? {
        ...
        try {
            ...
            try {
                ...
                metadataFile = dirUtils.moveToOutputDir(
                    metadataFile,
                    path,
                    MIME_METADATA,
                    customUserDir
